...
- **Cypress** (`cypress/tests/functional/OjsbrFilenameRename.cy.js`): the name of a published
  galley downloaded in pt_BR, en and es (OJS 3.4 has no language in the URL, so each download
  starts a session through `user/setLocale`); with a journal manager, the settings and their effect.
...
- Verified on OJS 3.4.0.10: workflow and galley downloads in pt_BR, en and es, both formats, both
  language settings, disabling and re-enabling, and the settings of 1.1.0.1 kept.
...
em `cypress/tests/functional/`, com os comandos da seção em inglês. Verificado no OJS 3.4.0.10:
downloads do fluxo editorial e de composições em pt_BR, en e es, os dois formatos, as duas opções
de idioma, desativar e reativar, mantendo as configurações da 1.1.0.1.
