...
/**
 * @file plugins/generic/ojsbrFilenameRename/tests/PluginTest.php
 *
...
class PluginTest extends TestCase
{
    ...
    {
        // A missing return type is accepted by php -l and is a fatal error when
        // the class is loaded next to the PKP version that declares it.
        foreach ([OjsbrFilenameRenamePlugin::class, OjsbrFilenameRenameSettingsForm::class] as $class) {
    ...
    public function testRequestedIdsComeFromTheQueryAndTheLastPathArgument(): void
    {
        $request = new class () extends Request {
            ...
            // OJS 3.4 declares getUserVar($key) without types.
        ...
        $pageRouter = new class () extends PageRouter {
            ...
            // OJS 3.4 declares getRequestedArgs($request) without types.
    ...
    public function testSettingsFormRejectsValuesOutsideTheChoices(): void
    {
        ...
        // OJS 3.4 keeps the session in PHP's own handler, which the CSRF and POST
        // checks start; under PHPUnit, after output was printed, that is an error.
        // Only the field checks are under test here.
        $form->_checks = array_values(array_filter($form->_checks, fn ($check) => $check instanceof \PKP\form\validation\FormValidatorInSet));
        $form->validate(false);
